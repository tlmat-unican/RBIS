...
NrRlcUm::DoTransmitPdcpPdu(Ptr<Packet> p)
{
    ...
    // std::cout << "Comparo m_txBufferSize + p (" << m_txBufferSize << " + " << p->GetSize() << ") con m_maxTxBufferSize (" << m_maxTxBufferSize << ")" << std::endl;
    // std::cout << "RLC[" << this << "] RNTI=" << m_rnti << " LCID=" << (uint32_t)m_lcid 
    //       << " Comparo m_txBufferSize + p (" << m_txBufferSize << " + " << p->GetSize() 
    //       << ") con m_maxTxBufferSize (" << m_maxTxBufferSize << ") porcentaje de ocupación: " << (double)m_txBufferSize / (double)m_maxTxBufferSize * 100 << "%" << std::endl;
    ...
    else
    {
        ...
        // // std::cout << "!!! Tx Buffer is full. RLC SDU discarded " << m_txBufferSize << " + " << p->GetSize() << " is greater than " << m_maxTxBufferSize << std::endl;
        NS_LOG_LOGIC("MaxTxBufferSize = " << m_maxTxBufferSize);
        NS_LOG_LOGIC("txBufferSize    = " << m_txBufferSize);
        NS_LOG_LOGIC("packet size     = " << p->GetSize());
        m_txDropTrace(p);

        // std::cout << "!!! RLC[" << this << "] Buffer full. Clearing old packets..." << std::endl;
        // // Mantener solo los últimos 10 paquetes (o un porcentaje)
        // size_t packetsToKeep = 10; // o m_txBuffer.size() * 0.1 (10%)
        // while (m_txBuffer.size() > packetsToKeep)
        // {
        //     auto discardedPacket = m_txBuffer.front().m_pdu;
        //     m_txBufferSize -= discardedPacket->GetSize();
        //     m_txDropTrace(discardedPacket);
        //     m_txBuffer.pop_front();
        // }
        // std::cout << "!!! RLC[" << this << "] New txBufferSize: " << m_txBufferSize << std::endl;
    }
