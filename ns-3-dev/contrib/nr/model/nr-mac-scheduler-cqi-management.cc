                                           ...
                                           uint16_t bandwidthInRbgs) const
{
    ...
    // Opcional: usar MCS de testing si está disponible
    // try 
    // {
    //     auto& drlInterface = Ns3DrlInterface::GetInstance();
    //     if (ueInfo->m_rnti <= 2) // Solo para UE 1 y 2
    //     {
    //         ueInfo->m_dlMcs = static_cast<uint8_t>(drlInterface.m_mcs_prueba[ueInfo->m_rnti-1]);
    //         std::cout << "Setting MCS UE " << ueInfo->m_rnti << " to " << static_cast<uint32_t>(ueInfo->m_dlMcs) << " with CQI expired" << std::endl;
    //     }
    // }
    // catch (...) { /* Usar MCS calculado normalmente */ }
    ...
    const std::unordered_map<uint16_t, std::shared_ptr<NrMacSchedulerUeInfo>>& ueMap) const
{
    ...
    for (const auto& itUe : ueMap)
    {
        ...
        if (ue->m_dlCqi.m_timer == 0)
        {
            ...
            auto& drlInterface = Ns3DrlInterface::GetInstance();
            ue->m_dlMcs = drlInterface.m_mcs_prueba[ue->m_rnti-1];
            
            
            // Usar la variable global o DRL interface para obtener el MCS de testing
            // uint8_t testMcs = 20; // Valor por defecto
            // // Primero intentar usar la variable global (más confiable)
            // if (SimulationDataLogger::g_mcs_test_value > 0 && SimulationDataLogger::g_mcs_test_value <= 31)
            // {
            //     testMcs = static_cast<uint8_t>(SimulationDataLogger::g_mcs_test_value);
            //     // std::cout << Simulator::Now().GetSeconds() << "\t | Using global MCS value: " << static_cast<uint32_t>(testMcs) << " for UE " << ue->m_rnti << std::endl;
            // }
            // else
            // {
            //     // Como respaldo, intentar obtener del DRL interface
            //     try 
            //     {
            //         auto& drlInterface = Ns3DrlInterface::GetInstance();
            //         if (true or ue->m_rnti <= 2 && drlInterface.GetUEsNumber() >= ue->m_rnti) // Solo para UE 1 y 2
            //         {
            //             double drlMcs = drlInterface.m_mcs_prueba[ue->m_rnti-1];
            //             if (drlMcs > 0 && drlMcs <= 31) // Validar rango
            //             {
            //                 testMcs = static_cast<uint8_t>(drlMcs);
            //                 // std::cout << "Using DRL interface MCS value: " << static_cast<uint32_t>(testMcs) << " for UE " << ue->m_rnti << std::endl;
            //             }
            //             else
            //             {
            //                 std::cout << "Invalid DRL MCS value (" << drlMcs << ") for UE " << ue->m_rnti << ", using default: " << static_cast<uint32_t>(testMcs) << std::endl;
            //             }
            //         }
            //     }
            //     catch (...)
            //     {
            //         std::cout << "DRL interface not available for UE " << ue->m_rnti << ", using default MCS: " << static_cast<uint32_t>(testMcs) << std::endl;
            //     }
            // }
            
            // ue->m_dlMcs = testMcs;
            // // std::cout << "Final: Setting DL MCS for UE " << ue->m_rnti << " to " << static_cast<uint32_t>(testMcs) << ", ue->m_dlMcs: " << static_cast<uint32_t>(ue->m_dlMcs) << std::endl;
        }
        else
        {
