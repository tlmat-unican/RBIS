...
class NrMacSchedulerOfdma : public NrMacSchedulerTdma
{
  ...
  private:
    ...
    /**
     * @brief Update the MCS of all UEs periodically
     */
    void UpdateMcsPeriodically() const;

    /**
     * @brief Initialize the MCS periodic scheduler
     */
    void StartMcsScheduler() const;

    /**
     * @brief Function that runs every 2 seconds to change the MCS
     */
    void ChangeMcsPeriodically() const;
    ...
    // Variables para control de MCS cada 2 segundos usando Simulator::Schedule
    mutable bool m_mcsSchedulerStarted; //!< Flag para saber si el scheduler de MCS ha iniciado
    mutable EventId m_mcsChangeEvent; //!< Event ID para el cambio periódico de MCS
