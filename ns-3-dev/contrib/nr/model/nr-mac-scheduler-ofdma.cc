...
NrMacSchedulerOfdma::NrMacSchedulerOfdma()
    : NrMacSchedulerTdma(),
      m_mcsSchedulerStarted(false)
{
    // // UpdateMcsPeriodically(); // Cambia MCS para todos los schedulers
}
...
NrMacSchedulerOfdma::AssignDLRBG(uint32_t symAvail, const ActiveUeMap& activeDl) const
{
    ...
    for (const auto& el : activeDl)
    {
        ...
        if (LOG_ENABLE) {
            for (auto ue_buffer: ueVector){
                auto ue = ue_buffer.first;
                SimulationDataLogger::LogRBGAllocation(ue->m_rnti,
                                                    ue->m_dlRBG.size(),
                                                    ue->m_dlMcs);
            }
        }
...
void
NrMacSchedulerOfdma::UpdateMcsPeriodically() const
{
    // Only start the scheduler the first time it is called
    if (!m_mcsSchedulerStarted)
    {
        std::cout << "Starting MCS scheduler for the first time at time: " << Simulator::Now().GetSeconds() << std::endl;
        StartMcsScheduler();
    }
}

void
NrMacSchedulerOfdma::StartMcsScheduler() const
{
    m_mcsSchedulerStarted = true;

    SimulationDataLogger::g_mcs_test_value = 28.0;
    SimulationDataLogger::g_last_mcs_change_time = Simulator::Now().GetSeconds();
    
    try 
    {
        auto& drlInterface = Ns3DrlInterface::GetInstance();
        // NOTE: fixed to 4 UEs for testing purposes
        for (int i = 0; i < 4; i++)
        {
            drlInterface.m_mcs_prueba[i] = 28.0;
        }        
    }
    catch (...)
    {
        NS_LOG_INFO("DRL Interface not available, continuing with global variable only.");
    }

    std::cout << "MCS initialized to 28 at time: " << Simulator::Now().GetSeconds() << std::endl;

    // Schedule the first change in 1 second
    m_mcsChangeEvent = Simulator::Schedule(Seconds(1.0), 
                                          &NrMacSchedulerOfdma::ChangeMcsPeriodically, 
                                          this);
}

void
NrMacSchedulerOfdma::ChangeMcsPeriodically() const
{    
    std::cout << "En ChangeMcsPeriodically() at time: " << Simulator::Now().GetSeconds() << std::endl;
    
    static std::random_device rd;
    static std::mt19937 gen(rd());
    static std::uniform_real_distribution<> dis(0.0, 1.0);

    // Also update in drl_itf if available
    try
    {
        auto& drlInterface = Ns3DrlInterface::GetInstance();
        // Update for all available UEs
        std::cout << "There are " << drlInterface.GetUEsNumber() << " UEs in DRL Interface." << std::endl;
        // for (int i = 0; i < drlInterface.GetUEsNumber(); i++)
        // NOTE: fixed to 4 UEs for testing purposes
        for (int i = 0; i < 4; i++)
        {
            double randomValue = dis(gen);
            drlInterface.m_mcs_prueba[i] = (randomValue < 0.15) ? 20 : 28;
            std::cout << "At time " << Simulator::Now().GetSeconds() 
                      << ", DRL Interface MCS for UE " << i 
                      << " changed to " << static_cast<int>(drlInterface.m_mcs_prueba[i]) 
                      << std::endl;
        }
    ...
    {
        // Continue only with global variable if no DRL interface
        std::cout << "ERROR" << std::endl;
    }

    // Schedule the next change in x seconds
    m_mcsChangeEvent = Simulator::Schedule(Seconds(1.0), 
                                          &NrMacSchedulerOfdma::ChangeMcsPeriodically, 
                                          this);
}
